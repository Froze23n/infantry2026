...
//微秒延时
static void bmi_delay_us(uint16_t us);
//温度计
static float BMI088_Read_Temperature(void);
...
uint8_t IMU_Init(void)
{
    ...
    if(state==BMI088_NO_ERROR){
        HAL_TIM_Base_Start_IT(&htim10);//1ms一次中断用于数据处理
...
static void IMU_Get_Data(void)
{
    ...
    imu_raw_data.ax = (tmp / ACCEL_SENSITIVITY_3);
    tmp = (int16_t)((acc_buff[3] << 8) | acc_buff[2]);
    imu_raw_data.ay = (tmp / ACCEL_SENSITIVITY_3);
...
static float BMI088_Read_Temperature(void)
{
	...
	HAL_GPIO_WritePin(CS1_ACCEL_GPIO_Port, CS1_ACCEL_Pin, GPIO_PIN_RESET);  // 拉低片选信号，开始传输
	//数据交换
	uint8_t txbyte = BMI088_TEMP_M | 0x80;  // 第一位为1表示读操作
	HAL_SPI_TransmitReceive(&hspi1, &txbyte, rxbuff, 1, BMI088_SPI_TIME_THRES);
	HAL_SPI_TransmitReceive(&hspi1, &txbyte, rxbuff, 1, BMI088_SPI_TIME_THRES);//额外的读取操作，筛选掉无效读数

	HAL_SPI_TransmitReceive(&hspi1, &txbyte, &rxbuff[0], 1, BMI088_SPI_TIME_THRES);
	HAL_SPI_TransmitReceive(&hspi1, &txbyte, &rxbuff[1], 1, BMI088_SPI_TIME_THRES);
	//结束
	HAL_GPIO_WritePin(CS1_ACCEL_GPIO_Port, CS1_ACCEL_Pin, GPIO_PIN_SET);  // 拉高片选信号，结束传输
	int16_t raw = (int16_t)((rxbuff[0] << 3) | (rxbuff[1] >> 5));
	if (raw > 1023) raw -= 2048;
	return (float)raw * 0.125f + 23.0f;
}
...
void IMU_Task(void)
{
	...
	//温度控制
	if (imu_raw_data.temperature < 40) {
